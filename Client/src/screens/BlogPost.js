...
function BlogPost() {
      ...
      return blogPost?.map((item)=>{
        return (
          <tr>
            ...
            <td>{item.author}</td>
            <td>
              <button className="btn btn-info m-1" data-target='#editBlogPost' onClick={() =>{setForm(item);}}  data-toggle='modal'>
                Edit
                </button> 
  ...
  return (
    ...
    <div className='row m-2 p-2'>
      <div className='col-9'>
        <h2 className='text-primary text-left'>BlogPost List</h2>
      </div>
      <div className='col-3'>
        <button className='btn btn-info form-control' data-target="#newBlogPost" data-toggle="modal">
          Add New BlogPost
        </button>
      </div>
    </div>
